<script lang="ts">
  ...
  const deltaDescription = $derived(
    [`${dir === 'up' ? 'increased' : 'decreased'} ${delta}`, deltaLabel]
      .filter(Boolean)
      .join(' '),
  )
</script>
...
>
  ...
  {:else}
    <div class="val" role="status" aria-live="polite">
      {#if prefix}<span class="small">{prefix}</span>{/if}{value}{#if suffix}<span class="small">{suffix}</span>{/if}
    </div>
    {#if delta}
...
<style lang="scss">
  .ss-metric {
    ...
    .delta {
      ...
      display: inline-flex; align-items: baseline; gap: var(--ss-s-1);
      &.positive { color: var(--ss-success); }
      &.negative { color: var(--ss-danger); }
      &.neutral  { color: var(--ss-fg-muted); }

      .period { color: var(--ss-fg-faint); }
      ...
      &.emphasis {
        ...
        &.positive { background: var(--ss-success-soft); }
        &.negative { background: var(--ss-danger-soft); }
        &.neutral  { background: var(--ss-hover); }
      }
    }

    .chart { margin-top: var(--ss-s-1); }
    ...
    .skeleton-val   { height: var(--ss-metric-val); width: 60%; }
    .skeleton-delta { height: var(--ss-ui-xs); width: 35%; }
  }
  ...
  @media (prefers-reduced-motion: reduce) {
    .ss-metric .skeleton { animation: none; }
  }
</style>
