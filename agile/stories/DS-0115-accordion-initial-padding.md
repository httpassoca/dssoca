...
## Acceptance criteria
- [x] The panel inner padding (`var(--ss-acc-body-py) var(--ss-acc-body-px)`) is applied unconditionally to the always-present inner wrapper that sits INSIDE the `0fr/1fr` grid clip, not toggled on the open state.
- [x] The grid clip (`overflow: hidden` + `min-height: 0` on the clipped wrapper) continues to hide the padded content while collapsed, so the collapsed item shows no padding bleed and measures 0 height.
- [x] Opening and closing animates the content as a single stable padded block — no reflow/snap of padding during the transition (verify visually at sm/md/lg).
- [x] Collapsed-state height is still exactly 0 (no residual padding height) so adjacent items butt cleanly against the `1px solid var(--ss-line)` separators.
- [x] `prefers-reduced-motion` behaviour is unchanged (instant collapse via `--ss-dur: 0ms`), padding still correct.
- [x] No new tokens; zero border-radius preserved; styling stays scoped SCSS using `--ss-*` tokens.
