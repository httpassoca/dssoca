...
- [ ] `--ss-gap-xs` documented in `docs/tokens.md` alongside `--ss-gap` / `--ss-gap-sm` (all three size values listed), matching the table format used for the other gap tokens.
- [ ] `src/lib/components/FileDrop.svelte` `<ul class="files">` rule uses `gap: var(--ss-gap-xs)` (replacing `var(--ss-gap-sm, 4px)`).
- [ ] Scope decision recorded: only the **list** (`ul.files`) outer gap changes to `--ss-gap-xs`; the per-file inner row gap (`.file { gap: var(--ss-gap-sm, 4px) }`) **stays on `--ss-gap-sm`** — the inner row separates name/size/remove and reads better with the slightly larger step; revisit only if visual harmony dictates.
- [ ] Visual check confirms a tighter selected-files list at sm/md/lg in both themes, with no regression to the `.zone`, `.prompt`, or `.clear` spacing.
- [ ] Zero border-radius preserved; tokens stay CSS custom properties (not Sass `$variables`) so the size-axis cascade resolves at runtime; `--ss-*` naming.
