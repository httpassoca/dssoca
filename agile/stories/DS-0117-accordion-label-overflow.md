...
## Acceptance criteria
- [ ] New prop `overflow?: 'truncate' | 'wrap'` on the Accordion `Props` interface, defaulting to `'wrap'`.
- [ ] `wrap` (default) reproduces today's exact behaviour: the label wraps across lines, no API/visual change for existing consumers.
- [ ] `truncate` constrains the title to a single line with an ellipsis: `white-space: nowrap; overflow: hidden; text-overflow: ellipsis` on `.title` (the required `min-width: 0` is already present on the flex title).
- [ ] The mode is applied to the `.title` element only (default header markup); when a custom `header` snippet is supplied, document that the consumer owns its own overflow handling.
- [ ] The prop is bindable-free (plain prop), typed, and the default keeps current rendering; both modes verified at sm/md/lg.
- [ ] Truncation never clips the chevron/indicator or the hint — the flex layout (`title` flex:1 1 auto, others flex:0 0 auto) keeps them visible.
- [ ] Zero border-radius preserved; scoped SCSS with `--ss-*` tokens; Svelte 5 runes for the prop.
