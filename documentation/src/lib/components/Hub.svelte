<script lang="ts">
  /**
   * The landing "workstation": a tiled field of live dssoca components behind
   * the hero. Each tile is dimmed and lifts to full opacity on hover (its own
   * card only). Every tile's props cycle on an independent random 1–4s timer so
   * the field feels alive — but each tile is a fixed-size, overflow-clipped box,
   * so a mutating prop never shifts the layout. Frozen under prefers-reduced-motion.
   *
   * Rendered client-only by the page (components never hit SSR/prerender).
   * Tile order + variant pools live in `$lib/hub-data` (so they're unit-tested).
   */
  ...
  import { TILES, V, BLEED } from '$lib/hub-data';
...
<style lang="scss">
  ...
  .tile.bleed .inner { padding: 0; align-items: stretch; justify-content: stretch; }
  .tile.bleed :global(.ss-side),
  .tile.bleed :global(.ss-topbar) { width: 100%; }
