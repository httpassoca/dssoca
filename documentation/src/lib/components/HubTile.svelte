...
{#snippet body()}
  <svelte:boundary>
    ...
    {:else if slug === 'sparkline'}
      <div class="w-full">
        <Sparkline data={v.data ?? []} label="req/min, last 6h" variant="area" fluid />
      </div>
    ...
    {:else if slug === 'heading'}
      <div class="heading-tile"><Heading level={3} size="sm">{v.t}</Heading></div>
    {:else if slug === 'container'}
      <Container><p class="cbody container-demo">centered, max-width content</p></Container>
    {:else if slug === 'textarea'}
      <div class="w-full"><Textarea label={v.label} value={v.val} rows={2} /></div>
...
<style lang="scss">
  ...
  .tile {
    ...
    // resting opacity matches the tile-in keyframe's end value (0.2) so the
    // entrance never "pops" brighter when the animation hands back to CSS
    opacity: 0.2;
  ...
  .tile[data-slug='sidebar'] :global(.ss-side) {
    width: 100%;
    height: 100%;
  }
  .tile[data-slug='topbar'] .content {
    align-items: flex-start;
  }
  .tile[data-slug='topbar'] :global(.ss-topbar) {
    width: 100%;
  }
  .tile[data-slug='log-stream'] :global(.ss-logs) {
    width: 100%;
    height: 100%;
  }
  ...
  // Heading tile: kill the display margin so the type centers in the cell.
  .heading-tile :global(.ss-heading) {
    margin: 0;
  }
  // Container tile: a dashed outline makes the invisible width wrapper legible.
  .container-demo {
    text-align: center;
