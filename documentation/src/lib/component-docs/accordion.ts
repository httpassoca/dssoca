...
<Accordion {items} defaultValue="shipping">
  ...
  props: [
    { name: 'items', type: 'AccordionItem[]', desc: 'Required. Sections — each `{ id, label, hint?, disabled? }`.' },
    { name: 'panel', type: 'Snippet<[AccordionItem]>', desc: 'Required. Renders the content for each section; receives the item.' },
    { name: 'header', type: 'Snippet<[AccordionItem]>', desc: 'Optional custom header content (replaces the default label + hint).' },
    { name: 'multiple', type: 'boolean', default: 'false', desc: 'Allow several sections open at once; otherwise opening one closes the others.' },
    { name: 'value', type: 'string | string[]', desc: 'Controlled, bindable open state — an id (single) or list of ids (`multiple`).' },
    { name: 'defaultValue', type: 'string | string[]', desc: 'Initial open section(s) when uncontrolled.' },
    { name: 'onChange', type: '(value) => void', desc: 'Fired with the new open value whenever a section toggles.' },
    { name: 'headingLevel', type: '1 | 2 | 3 | 4 | 5 | 6', default: '3', desc: 'Heading level wrapping each header button (document outline).' },
    { name: 'idBase', type: 'string', desc: 'Base id used to namespace the generated header/panel ids.' },
    SIZE_PROP,
  ],
  notes:
