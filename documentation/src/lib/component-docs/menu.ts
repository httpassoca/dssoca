  ...
  props: [
    {
      ...
      type: 'MenuItem[]',
      default: '[]',
      desc: 'Rows in the panel. Each: { id, label, icon?, leading?, swatch?, emoji?, disabled?, selected?, onSelect? }. A defined `selected` on any item switches the menu to a selection (radio) group. Leading visual per item: `leading` (a Svelte 5 Snippet for arbitrary markup), `swatch` (any CSS color, rendered as a token-sized square color chip — zero radius), or `emoji` (a text glyph, e.g. a flag); precedence when several are set is leading → swatch → emoji → icon. All leading visuals share the `--ss-icon` footprint so mixed menus align, and are aria-hidden (the label carries the semantics).',
    },
    {
      name: 'open',
      type: 'boolean',
      default: 'false',
      desc: 'Bindable open state. Omit `bind:` for an uncontrolled menu the trigger toggles itself.',
    },
    {
      name: 'onSelect',
      type: '(id: string) => void',
      desc: "Fired when a row is activated, with its `id` (after the item's own `onSelect`).",
    },
    {
      name: 'align',
      type: "'start' | 'end'",
      default: "'start'",
      desc: 'Horizontal edge the panel aligns to relative to the trigger.',
    },
    {
      name: 'side',
      type: "'bottom' | 'top'",
      default: "'bottom'",
      desc: 'Vertical side the panel opens toward.',
    },
    { name: 'label', type: 'string', desc: 'Accessible name for the panel (`role="menu"`).' },
    {
      name: 'children',
      type: 'Snippet',
      desc: 'Trigger content rendered inside the toggle button.',
    },
    SIZE_PROP,
  ],
  notes:
