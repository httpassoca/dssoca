...
/>`,
    props: [
      ...
      { name: 'tabs', type: 'Array<string | { id; label?; href? }>', desc: 'Tab strip (has a built-in default set). Strings are shorthand for `{ id, label: id }`; `label` defaults to `id`; tabs with `href` render as real `<a href>` links (SSR-friendly).' },
      { name: 'user', type: 'string', desc: 'Current user label.' },
      { name: 'onTab', type: '(tab: string) => void', desc: 'Fired with the tab id on activation (also for `href` tabs, as an SPA fallback).' },
      { name: 'onCommand', type: '() => void', desc: 'Fired when the command menu is opened (chip click or ⌘/Ctrl+K). The ⌘K chip and the shortcut only exist when this is provided.' },
      ...
      { name: 'stats', type: '{ key; value; title? }[]', desc: 'Right-aligned stat segments; defaults to built-in homelab stats. Pass `[]` to remove.' },
      { name: 'services', type: 'boolean | { up: number; total: number }', default: 'true', desc: 'Services status segment: `true` keeps the built-in 6/7 demo summary, an object renders real numbers, `false` removes it.' },
      { name: 'clock', type: 'boolean', default: 'true', desc: 'Show the live clock segment; `false` removes it.' },
