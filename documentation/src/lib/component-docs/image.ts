...
/>`,
  props: [
    { name: 'src', type: 'string', desc: 'Image URL (fallback <img> source).' },
    {
      name: 'alt',
      type: 'string',
      desc: 'Required alternative text (WCAG 1.1.1). Use "" only for purely decorative images.',
    },
    ...
    { name: 'sizes', type: 'string', desc: 'sizes hint paired with srcset.' },
    {
      ...
      type: 'ImageSource[]',
      default: '[]',
      desc: 'Responsive <picture> sources: { srcset, media?, type?, sizes? }.',
    },
    {
      name: 'width',
      type: 'number',
      desc: 'Intrinsic width (px) — feeds the aspect-ratio box and the img attr.',
    },
    {
      name: 'height',
      type: 'number',
      desc: 'Intrinsic height (px) — feeds the aspect-ratio box and the img attr.',
    },
    {
      name: 'ratio',
      type: 'number',
      desc: 'Aspect ratio (w / h) to reserve space before load. Wins over width/height.',
    },
    {
      name: 'fit',
      type: "'cover' | 'contain' | 'fill' | 'none' | 'scale-down'",
      default: "'cover'",
      desc: 'object-fit inside the reserved ratio box.',
    },
    {
      name: 'eager',
      type: 'boolean',
      default: 'false',
      desc: 'Eager-load above-the-fold (loading="eager", decoding="auto", fetchpriority="high").',
    },
    {
      name: 'lightbox',
      type: 'boolean',
      default: 'false',
      desc: 'Open the built-in dialog overlay on click (role="dialog", aria-modal, focus trap, Esc to close, restores focus).',
    },
    {
      name: 'caption',
      type: 'string',
      desc: 'Caption rendered in a <figcaption> under the image.',
    },
    SIZE_PROP,
  ],
  notes:
