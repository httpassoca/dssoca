...
<Link variant="button" href="/get-started">get started</Link>`,
  props: [
    { name: 'href', type: 'string', desc: 'Destination URL.' },
    {
      ...
      type: "'inline' | 'button'",
      default: "'inline'",
      desc: 'Animated-underline link (inline) or solid primary button-styled link (button).',
    },
    {
      name: 'external',
      type: 'boolean',
      desc: 'Open in a new tab + safe rel + external glyph. Auto-detected for absolute http(s) URLs when left undefined; pass false to force same-tab.',
    },
    {
      name: 'disabled',
      type: 'boolean',
      default: 'false',
      desc: 'Render the link inert: the href is dropped (no navigation, out of the tab order via tabindex="-1"), link semantics are kept with role="link" + aria-disabled="true", forwarded onclick handlers do not fire, and the external affordances are suppressed.',
    },
    ...
    { name: 'children', type: 'Snippet', desc: 'Link label.' },
    {
      ...
      desc: 'Any remaining native anchor attributes / handlers are forwarded (including an explicit target/rel that override the external defaults).',
    },
  ],
  notes:
