...
describe('Image', () => {
  ...
  describe('lightbox', () => {
    ...
    it('focus trap excludes disabled focusables (DS-0078)', async () => {
      const { container } = render(Image, { src: SRC, alt: 'x', lightbox: true })
      await fireEvent.click(container.querySelector('button.trigger')!)
      const dialog = document.querySelector('.ss-image-lightbox') as HTMLElement
      const body = dialog.querySelector('.dialog-body') as HTMLElement
      // Simulate disabled chrome inside the overlay, after the close button.
      const d1 = document.createElement('button')
      d1.disabled = true
      const d2 = document.createElement('button')
      d2.setAttribute('aria-disabled', 'true')
      body.append(d1, d2)

      const close = dialog.querySelector('button.close') as HTMLElement
      close.focus()
      const evt = new KeyboardEvent('keydown', { key: 'Tab', bubbles: true, cancelable: true })
      close.dispatchEvent(evt)
      // close is both first and last *enabled* focusable, so the trap wraps
      // (preventDefault + refocus) instead of treating a disabled button as
      // the last tab stop.
