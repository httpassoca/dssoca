...
describe('EmptyState', () => {
  ...
  it('applies ariaLabel as the accessible name of the live region', () => {
    const { container } = render(EmptyState, { title: 'Nothing', ariaLabel: 'Search results' })
    expect(container.querySelector('.ss-empty')).toHaveAttribute('aria-label', 'Search results')
  })

  it('renders no aria-label by default (backwards compatible)', () => {
    const { container } = render(EmptyState, { title: 'Nothing' })
    expect(container.querySelector('.ss-empty')).not.toHaveAttribute('aria-label')
  })
