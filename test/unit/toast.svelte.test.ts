...
// Timer hardening (DS-0071): tests that arm fake timers must restore real
// timers even when an assertion throws mid-test (an inline trailing
// `vi.useRealTimers()` never runs on failure and poisons later tests/files).
// Guarantee the restore once, file-wide.
afterEach(() => {
  vi.useRealTimers()
})
