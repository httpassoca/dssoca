...
describe('SegmentedControl', () => {
	...
	it('selects on click and fires onChange', async () => {
		...
		const { container } = render(SegmentedControl, { options: OPTS, value: 'list', onChange, label: 'View' });
		const segs = container.querySelectorAll<HTMLElement>('.segment');
	...
	it('applies the full class when fullWidth is set', () => {
		const { container } = render(SegmentedControl, { options: OPTS, fullWidth: true, label: 'View' });
	...
	it('has no axe violations', async () => {
		const { container } = render(SegmentedControl, { options: OPTS, value: 'grid', label: 'View mode' });
