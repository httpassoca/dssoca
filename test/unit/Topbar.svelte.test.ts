...
describe('Topbar', () => {
  ...
  it('renders a skip link as the first focusable child, defaulting to #main', () => {
    ...
    const focusables = container.querySelectorAll('a[href], button, [tabindex]')
  ...
  it('labels the nav landmark via the ariaLabel prop (default stays "Primary")', () => {
    const { container } = render(Topbar, { ariaLabel: 'Main navigation' })
    expect(container.querySelector('nav.ws')).toHaveAttribute('aria-label', 'Main navigation')
